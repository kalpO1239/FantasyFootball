#!/usr/bin/env python3
"""
Generate Enhanced QB Projections with Minimax Theory and Markov Chains
This script runs the enhanced predictor for all QB players and creates visualizations
"""
...
import sys
from enhanced_qb_predictor import enhanced_prediction, get_base_prediction
...
def get_all_qb_players(artifacts_dir: Path) -> list:
    """Get list of all QB players from the training table"""
    training_table = pd.read_parquet(artifacts_dir / "qb_training_table.parquet")
    players = training_table['PLAYER'].unique().tolist()
    return sorted(players)

def generate_all_projections(artifacts_dir: Path = Path("./artifacts_qb")) -> pd.DataFrame:
    """
    Generate enhanced projections for all QB players
    """
    print("Loading QB player data...")
    players = get_all_qb_players(artifacts_dir)
...
def create_visualizations(df: pd.DataFrame, output_dir: Path = Path("./enhanced_qb_projections")):
    """
    ...
    plt.title('Top 30 Enhanced QB Projections (Minimax + Markov Chains)', 
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'top_30_enhanced_qb_projections.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title('Base vs Enhanced QB Projections (Top 25)', 
              fontsize=16, fontweight='bold')
    plt.xlabel('Players', fontsize=12)
    ...
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'base_vs_enhanced_qb_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    # Component breakdown
    components = ['minimax_adjustment', 'markov_adjustment', 'performance_penalty', 
                  'passing_consistency']
    component_labels = ['Minimax', 'Markov', 'Performance', 'Passing']
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'qb_adjustment_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'qb_distribution_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()

def main():
    """Main function to generate enhanced QB projections"""
    artifacts_dir = Path("./artifacts_qb")
    output_dir = Path("./enhanced_qb_projections")
    
    print("=== Enhanced QB Projection Generator ===")
    ...
    # Save to CSV
    output_dir.mkdir(exist_ok=True)
    csv_path = output_dir / "enhanced_qb_projections.csv"
    ...
    # Display summary
    print(f"\n=== Summary ===")
    print(f"Total QB players processed: {len(df)}")
    ...
    print(f"\nTop 10 Enhanced QB Projections:")
    top_10 = df.head(10)
